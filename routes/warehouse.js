...
router.get(
    ...
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
      ...
      return res.json(await warehouses.findAndCount({
        attributes: [
          'id', 'name', 'email', 'phoneNumber', 'warehouseType',
          'primaryWarehouseId',
          [
            sequelize.literal(
          ...
          'province', 'address', 'latitude', 'longitude', 'areaCode',
          'buildingName', 'city', 'description', 'updatedBy', 'createdAt'
        ],
...
router.get('/outlets', asyncErrorHandlerMiddleWare(async (req, res, next) => {
             return res.status(200).json(await warehouses.findAll({
               attributes: ['id', 'name'],
               where: {warehouseType: 'Teritary', isActive: true}
             }));
           }));

router.get(
    '/:primaryWarehouseId/branches', [auth.authenticate()],
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
      const warehouse = await warehouses.findAll({
        attributes: [
          ...
          'primaryWarehouseId',
          [
            sequelize.literal(`(select name from warehouses where id = ${
...
router.get(
    '/:secondaryWarehouseId/outlets', [auth.authenticate()],
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
      const warehouse = await warehouses.findAll({
        attributes: [
          'id', 'name', 'email', 'phoneNumber', 'warehouseType',
          ...
          [
            sequelize.literal(`(select name from warehouses where id = ${
                req.params.secondaryWarehouseId})`),
