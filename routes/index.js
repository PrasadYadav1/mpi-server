...
const allowedExt =
    ['.js', '.ico', '.css', '.png', '.jpg', '.woff2', '.woff', '.ttf', '.svg'];
...
const reqValidation = (req, res, next) => {
  ...
  if (validRes.isValid)
    ...
    res.status(400).json(validRes.errors);
};
const bcrypt = require('bcryptjs');
router.post(
    '/api/token', asyncErrorHandlerMiddleWare(async (req, res, next) => {
      const hashedPassword = bcrypt.hashSync(req.body.password, 10);
      let user = await users.find({
        attributes: [
          'id', 'userName', 'password', 'firstName', 'lastName', 'email',
          'userRole', 'designation', 'avatar', 'mobileNumber', 'warehouseId',
          'branchId', 'outletIds'
        ],
      ...
      if (user && (await bcrypt.compare(req.body.password, user.password))) {
        res.json({
          token: jwt.encode(
              {id: user.id, datetime: moment().format('YYYY-MM-DD h:mm:ss a')},
              cfg.jwtSecret),
          users: {
            id: user.id,
            ...
            avatar: user.avatar ?
                'https://' + req.hostname + '/' + user.avatar :
                null,
...
router.get(
    '/api/sendMail', asyncErrorHandlerMiddleWare(async (req, res, next) => {
