...
router.get(
    '/list', [auth.authenticate(), reqQueryValidate(pagination)],
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
      ...
      if (result) {
        ...
        whereStatement = {
          isActive: true,
          [propertyName]:
              propertyName === 'warehouseId' ? {$eq: parseInt(propertyValue)} :
                                               {$iLike: `%${propertyValue}%`}
        };
      }

      const limit = parseInt(req.query.pageSize);
      return res.json(await uservisitlocations.findAndCount({
        attributes: [
          'id',
          [
            sequelize.literal(
...
router.get(
    '/', asyncErrorHandlerMiddleWare(async (req, res, next) => {
      ...
      const customerCount = await customers.findAll(
          {attributes: ['id'], where: {isActive: true}});
      const companyCount = await companies.findAll(
          {attributes: ['id'], where: {isActive: true}});
      const productCount =
          await products.findAll({attributes: ['id'], where: {isActive: true}});
      ...
      const orderCount =
          await orders.findAll({attributes: ['id'], where: {isActive: true}});
      const offerCount =
          await offers.findAll({attributes: ['id'], where: {isActive: true}});
      const data = {
        ...
        pendingPreOrderCount:
            preOrderCount.length - approvedPreOrderCount.length,
...
router.get(
    '/visit', asyncErrorHandlerMiddleWare(async (req, res, next) => {
