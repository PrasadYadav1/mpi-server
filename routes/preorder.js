...
router.get(
    ...
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
      const fromDateDefault = req.query.fromDate === 'null' ||
          req.query.fromDate === '' || req.query.fromDate === undefined;
      const toDateDefault = req.query.toDate === 'null' ||
          req.query.toDate === '' || req.query.toDate === undefined;

      const fromDateData = req.query.fromDate != 'null' &&
          req.query.fromDate != '' && req.query.fromDate != undefined;

      const toDateData = req.query.toDate != 'null' && req.query.toDate != '' &&
          req.query.toDate != undefined;

      const orderByDefault = req.query.orderBy === 'null' &&
          req.query.orderBy === '' && req.query.orderBy === undefined;
      const orderByData = req.query.orderBy != 'null' &&
          req.query.orderBy != '' && req.query.orderBy != undefined;
      let orderBy = 'DESC';
      if (orderByDefault) {
      ...
      if (result) {
        ...
        whereStatement = {
          isActive: true,
          [propertyName]:
              propertyName === 'customerId' ? {$eq: parseInt(propertyValue)} :
                                              {$iLike: `%${propertyValue}%`}
        };
      }

      const limit = parseInt(req.query.pageSize);
      return res.json(await preOrders.findAndCount({
        attributes: [
          'id', 'preorderConfirmed', 'preOrderNumber', 'dateOfDelivery',
          'customerId',
          [
            sequelize.literal(
          ...
          'discount', 'amount', 'totalAmount', 'isApproved',
          [
            sequelize.literal(
          ...
          [
            sequelize.literal(
                `( select "firstName" || ' ' || "lastName"  from "users" AS u where u.id = preOrders."createdBy")`),
            'salesAgentName'
          ],
...
router.post(
    '/', [auth.authenticate()],
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
      const preOrderCount = await preOrders.count({where: {isActive: true}});
      const customerData = await customers.findOne(
          {where: {isActive: true, id: req.body.customerId}});
      ...
      if (preOrder) {
        let strc = req.body.preorderProducts.map(function(n, i) {
          (n.preorderId = preOrder.dataValues.id),
              (n.createdBy = req.user.userId), (n.updatedBy = req.user.userId),
              (n.isActive = true);
          return n;
        });
      ...
      const transporter = nodemailer.createTransport({
        service: 'gmail',
        auth:
            {user: 'krishnarao.inturi@technoidentity.com', pass: 'Design_20'}
      });
      const mailOptions = {
        ...
        html: `<h1>Pre Order Number: ${
            preOrder.dataValues.preOrderNumber}</h1><p>Date of Delivery: ${
            preOrder.dataValues.dateOfDelivery}</p>`
      };
...
router.get(
    ...
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
      ...
      // var decodedImage = new Buffer(product.digitalSignature,
      // 'base64').toString(
      //   'binary'
      // );
...
router.get(
    '/:pId', [auth.authenticate()],
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
      const preOrder = await preOrders.update(
          {preorderConfirmed: true, updatedBy: req.user.userId},
...
router.put(
    '/isapproved/:id', [auth.authenticate()],
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
      const preOrder = await preOrders.update(
          {
...
// 	   var encodedImage = new Buffer(req.file.filename,
// 'binary').toString('base64'); 	   console.log(encodedImage) 	   const
// preOrder = await preOrders.update(
// 		{
...
router.put(
    '/:id/digitalsignature', [auth.authenticate()],
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
      const preOrder = await preOrders.update(
          {
...
router.delete(
    '/:preOrderId', [auth.authenticate()],
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
      const preOrder = await preOrders.update(
          {isActive: false, updatedBy: req.user.userId},
          {where: {id: req.params.preOrderId}});
      return res.sendStatus(200);
    }));

router.post(
    '/:preorderId/preorderproducts', [auth.authenticate()],
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
...
router.put(
    '/:id/preorderproducts', [auth.authenticate()],
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
...
router.delete(
    '/:preOrderProductId/preorderproducts', [auth.authenticate()],
    asyncErrorHandlerMiddleWare(async (req, res, next) => {
      const preOrder = await preorderProducts.update(
          {isActive: false, updatedBy: req.user.userId},
          {where: {id: req.params.preOrderProductId}});
      return res.sendStatus(200);
    }));
